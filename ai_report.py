...
def holdings_table_html(summary: list, krw: bool = False, chart_cid: str | None = None,
    ...
    totals={"strategy","bench","index_name"} — 누적수익률(보유+청산 포함) vs 지수.
    realized={"n","avg_pct"}(holdings.realized_summary()) — 이미 청산된 매매만의 평균 실현수익률.
    2026-07-26(지호 님 지적 — "수익률에 예전에 사고 판 것도 포함되는거지?"): 그땐 totals가
    보유 중인 바스켓만의 스냅샷이라 아니었음 — 별도 realized 줄로 혼동을 줄였었다.
    2026-09-23(지호 님 재지적 — "실현 수익은 포함해야지" → "누적수익률 그래프에도
    반영해라"): totals 자체가 이제 holdings.portfolio_series(..., realized_trades=...)로
    계산돼 청산분까지 동일비중으로 포함한다(청산 종목은 매도일까지 실가격, 이후 그 실현
    수익률로 고정) — realized 줄은 "청산만 따로 보면 어땠나"를 보여주는 별개 지표로 유지.
    ...
    if totals:
        ...
        totals_html = (
            f'<div style="font-size:13px;margin:2px 0 2px">전체 수익률(보유+청산 포함) '
    ...
    if realized:
        ...
        totals_html += (
            f'<div style="font-size:13px;margin:0 0 6px;color:#6b7280">(참고) 청산 완료 {realized["n"]}건 '
