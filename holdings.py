...
동작: 매일 '신규 매수'로 뽑힌 종목을 가상 보유목록에 자동 편입하고,
  ...
  · 정기 재평가: 보유 REEVAL_DAYS(미국)/KR_REEVAL_DAYS(한국) 경과 후 당일 팩터 후보풀 밖이면
    정리(현재 유일하게 살아있는 매도 트리거, 가격 개입 없음). 21조합 champion(exit_time6m)은
    '고정 6개월 청산'이라 이 규칙(기간 경과 AND 후보풀 이탈)과 같지 않다 — 이 규칙 자체의 검증은
    backtest_exec의 exit_live(research/us/us_full_stack_exec_validation.py, 2026-09-24 추가).
...
쪽)대로 **가격 개입 없음을 그대로 유지**(HISTORY.md §3 Stage 6). 진입가 -25% 트레일링만
