...
CORE_ASSETS = [
    ...
    ("ETH",    "이더리움",   "ETH-USD", "crypto_eth", ("coin",)),
]
...
PARAMS = {
    ...
    # 이진 노출로만 검증됐기 때문). 통계적으로 완전히 증명된 값은 아님 — HISTORY.md 참고.
    "bond":   {"trend_ma": 150, "band": 0.0,  "confirm": 3, "mom": "12_1", "dip_ma": 20, "vol_target": 0.15},
    # 2026-08-24(지호 님 결정): 이더리움 전용 — 비트코인 파라미터(120일선)를 튜닝 없이
    # 그대로 썼던 걸 교체(HISTORY.md §11-1/§11-2 검증 근거). 30일선·밴드0%·확인1일(순수
    # 돌파)이 무작위매매 대비 샤프 유의(99.8%ile)·시대분리(2구간 모두 승)·비용스트레스
    # (100bp에서도 매수후보유 이김)까지 통과한 실제 백테스트 조합 그대로 반영 — band·
    # confirm은 이 트렌드 판정에 한해 백테스트와 정확히 일치. mom·dip_ma·vol_target은
    # crypto 기본값을 그대로 물려받았고 이 조합 자체(30일선+3개월모멘텀 게이팅)는 별도
    # 검증 안 됨 — 모멘텀은 레짐 자체를 안 바꾸고 신규매수 라벨만 더 보수적으로 바꾸는
    # 요소라 리스크는 낮다고 판단(하방 위험 아님). ⚠ 짝지은 부트스트랩은 아직 0을 신뢰
    # 구간에 포함(통계적으로 완전히 확정된 건 아님, §11-2) — 다음 재검증 때 재확인할 것.
    "crypto_eth": {"trend_ma": 30, "band": 0.0, "confirm": 1, "mom": "3m", "dip_ma": 20, "vol_target": 0.40},
}
