...
ASSETS = [
    ...
    ("US_STOCK", "미국 주식",   "SPY",
     "핵심자산(코어) 50% SPMO(S&P500 모멘텀, 시가총액 가중) + 위성자산(새틀라이트) 50% 미장 메일 "
     "개별종목 추천 권장 — 2018~2026 계좌 단위 백테스트에서 이 50:50이 S&P500보다 수익률·샤프가 "
     "모든 구간에서 높았습니다(개별종목만으로는 소수 연도 의존이 커서 5개 연도는 S&P500에 짐). "
     "나스닥100을 꾸준히 이긴다는 근거는 없고, 통계적 확정도 아닙니다(HISTORY.md §13·§17)."),
    ("KR_STOCK", "한국 주식",   "^KS11",
     "핵심자산(코어) 65% KODEX 200 등 + 위성자산(새틀라이트) 35% 국장 메일 개별종목 추천"
     "(저평가·배당주) 권장 — 요즘 코스피는 삼전·하이닉스 쏠림으로 지수 자체가 오르는 장이라 "
     "개별종목만으로 지수를 이기긴 어렵습니다. 지수는 코어로 그대로 담고 저평가·배당주를 "
     "새틀라이트로 보완하는 구조가 위험 대비 수익(샤프지수)과 하락폭(MDD) 양쪽에서 "
     "더 안정적이었습니다(8년 백테스트). 참고: 새틀라이트 종목 수(3~15개 비교)보다 이 "
     "65:35 비중 자체가 성과를 갈랐습니다."),
