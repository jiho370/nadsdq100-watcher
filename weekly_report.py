...
ASSETS = [
    ...
    ("KR_STOCK", "한국 주식",   "^KS11",
     "코어 65% KODEX 200 등 + 새틀라이트 35% 국장 메일 개별종목 추천(저평가·배당주) 권장 — "
     "요즘 코스피는 삼전·하이닉스가 끌어올리는 장이라 개별종목만으로 지수를 이기긴 어려워서, "
     "지수는 그대로 담고(코어) 저평가·배당주로 보완하는(새틀라이트) 방식이 검증에서 더 "
     "안정적이었습니다. (참고: 새틀라이트를 몇 종목 담을지는 큰 차이가 없었고, 이 65:35 "
     "비율 자체가 핵심입니다.)"),
