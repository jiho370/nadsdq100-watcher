...
**알려진 한계(정직하게 공개)**
- 팩터 가중치(1:2:2) 자체는 665개 대안 조합을 통계적으로 유의하게 이긴 적이 없다(PBO 22%·DSR 0.88, 현재도 미달) — "이게 최적"이 아니라 "다른 조합이 확실히 낫다는 증거가 없어서" 유지 중.
- floor+100일선 필터까지 포함한 최종 선정+분할매수+청산규칙 전체를 하나로 합쳐 검증하면 표본이 33→18건으로 줄어 PBO는 낮지만(0~60%, 비교 범위에 따라 다름) DSR이 아직 0.95 기준 미달 — 방향은 기존 결론과 일치하나 표본이 더 쌓여야 통계적으로 확정된다(`research/us/us_full_stack_exec_validation.py`, 2026-09-24).
