...
ENTRY_RULES = ["entry1_full", "entry2_pullback2"]
EXIT_RULES = ["exit1_trail20", "exit2_atr2stage", "exit3_support2stage"]
...
def _simulate_trade(panel, ma20, ma200, atr, sym, entry_day, entry_rule, exit_rule):
    ...
    elif exit_rule == "exit2_atr2stage":
...
def run_exec(panel, spy, funds, pit, rebal_days=63, topn=15, cost=None):
    ...
    ma20, ma200, atr = _ma(panel, 20), _ma(panel, 200), _atr_close(panel)
    spy = spy.reindex(panel.index).ffill() if spy is not None else None
    n = len(panel)
    ps = list(range(BW.LOOKBACK, n - MAX_HOLD - 1, rebal_days))
    if not ps:
    ...
    for p in ps:
        basket = _select_basket(panel, p, funds, cross, pit, weights, topn)
        if not basket:
            continue
        date = panel.index[p].date().isoformat()
        ...
        for combo in combos:
            ...
            evs = []
            for sym in basket:
                r = _simulate_trade(panel, ma20, ma200, atr, sym, entry_day, entry_rule, exit_rule)
                if r is None:
                    continue
    ...
    payload = {"as_of": panel.index[-1].date().isoformat(), "weights_used": weights,
              ...
              "baseline": "entry1_full__exit1_trail20 (현행 시스템과 동일 규칙)",
...
def self_test():
    ...
    ma20, ma200, atr = _ma(p2, 20), _ma(p2, 200), _atr_close(p2)

    r = _simulate_trade(p2, ma20, ma200, atr, "CRASH", 210, "entry1_full", "exit1_trail20")
    assert r["stop"] and r["exit_price"] < r["entry_price"], f"급락 경로인데 스톱 미발동: {r}"

    flat_r = _simulate_trade(p2, ma20, ma200, atr, "FLAT", 210, "entry2_pullback2", "exit1_trail20")
