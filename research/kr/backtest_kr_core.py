...
유동성 필터)를 검증한다:
  ...
  3) 비용: backtest_costs.CostModel 에 전부 위임한다 — 매도 증권거래세율의 단일 출처는
     backtest_costs.KR_SELL_TAX 이고, 여기서 값을 다시 적지 않는다(2026-09-10 수정: 예전엔
     이 자리에 "코스피 0.15%+코스닥 0.20%"라고 적혀 있었으나 실제 상수는 양쪽 동일이라
     주석만 어긋나 있었다 — 세율은 개정되므로 문서가 아니라 상수를 볼 것).
     스몰캡 유니버스라 보수적으로 "kosdaq" 기준을 KOSPI 종목에도 같이 적용한다.
...
한계:
 · ⚠ 가격제한폭(상한가/하한가) 체결 모델 **미구현** — KR_STRATEGY_OPTIONS.md §5-1이 요구한
   항목이지만 이 파일은 구현하지 않았다(2026-09-10 확인). §5-1은 "상한가 도달 시 매수 체결
   불가(매도 잔량 0), 하한가 시 매도 불가"를 요구하며 "테마/이벤트 전략에서 이걸 안 하면
   수익이 2~3배 뻥튀기된다 — 제일 좋은 트레이드가 제일 체결 안 되는 구조"라고 경고한다.
   이 파일이 검증하는 §2-D(월간 리밸·스몰캡 밸류)는 상한가 직행 종목을 노리는 전략이 아니라
   영향이 §5-1이 상정한 테마/이벤트 트랙보다는 작을 것으로 보이나, **정량화된 적은 없다.**
   따라서 여기 결과는 "상한가 미체결분만큼 낙관 쪽으로 치우쳐 있을 수 있다"를 전제로 읽을 것.
   테마·이벤트·단기반전 트랙(§3·§4)으로 확장할 때는 이 레이어를 먼저 구현해야 한다.
 · pykrx 관리종목 '지정 이력' API 부재 → 거래대금·펀더멘탈 필터로 근사(§5-3 완전판은
...
def run(years=8, save=True):
    ...
    payload = {"as_of": dates[-1], "screen": f"PBR≤{PBR_MAX}·0<PER≤{PER_MAX}·거래대금≥{MIN_TRADING_VALUE:.0e}"
               ...
               "limitations": ["가격제한폭(상한가/하한가) 체결 모델 미구현 — KR_STRATEGY_OPTIONS.md §5-1 요구사항 "
                               "미이행. 상한가 미체결분이 반영되지 않아 결과가 낙관 쪽으로 치우칠 수 있음(영향 미정량)",
                               "3년연속흑자·부채비율 필터 미적용(DART 필요) — PBR·PER·거래대금으로 축소",
