...
NOTABLE_NEWS_HOURS = float(os.environ.get("NOTABLE_NEWS_HOURS", "48"))  # 3) 뉴스 트리거: 최근 N시간 내 발행만 인정(오래된 뉴스 제외)
ALLOW_RESEND   = os.environ.get("ALLOW_RESEND", "0") == "1"        # 1=같은 세션 재실행도 재발송(개발/잦은 점검용). 완성 후 0으로.
# ── 펀더멘탈 섹션(엄격 교집합): 섹터대비 저평가 + 확실한 추세 ──
FUND_ROE_MIN     = float(os.environ.get("FUND_ROE_MIN",   "0.15"))  # 퀄리티 하한 ROE(절대)
FUND_RELPE_MAX   = float(os.environ.get("FUND_RELPE_MAX", "1.0"))   # PER ÷ 섹터중앙값 상한(1.0=섹터 중앙값 이하만 통과)
FUND_RELPE_RELAX = float(os.environ.get("FUND_RELPE_RELAX","1.3"))  # 후보 부족 시 밸류게이트 1회 완화 상한
FUND_MOM_MIN     = float(os.environ.get("FUND_MOM_MIN",   "0"))     # 6개월 모멘텀 최소(%) — 추세 확인 요건
...
def fetch_news_flags(symbols: list[str]) -> dict[str, bool]:
    """3섹션 뉴스 트리거: '최근 NOTABLE_NEWS_HOURS 시간 내 발행된' 뉴스가 있을 때만 True.
    오래된/상시 노출 뉴스로 인한 오탐을 줄이기 위해 발행시각으로 필터링한다.
    yfinance 신·구 스키마(providerPublishTime epoch / content.pubDate ISO)를 모두 처리. 실패는 조용히 False."""
    _require_yf()
    cutoff = datetime.now(timezone.utc) - timedelta(hours=NOTABLE_NEWS_HOURS)
    flags = {}
    for sym in symbols:
        fresh = False
        try:
...
def _news_pub_dt(item: dict):
    """yfinance 뉴스 1건의 발행시각을 timezone-aware UTC datetime 으로. 못 구하면 None."""
    if not isinstance(item, dict):
        return None
    # 구 스키마: providerPublishTime = epoch seconds
    ...
    if ts:
        try:
            ...
            pass
    # 신 스키마: item["content"]["pubDate"/"displayTime"] = ISO8601 문자열
    content = item.get("content") if isinstance(item.get("content"), dict) else {}
    for key in ("pubDate", "displayTime"):
        s = content.get(key) or item.get(key)
        if not s:
            continue
        try:
...
def pick_trend(ind_map: dict[str, dict], sector_map: dict[str, str],
               spy_close: pd.Series, exclude: set | None = None, n: int = SECTION_N) -> list[tuple]:
    """1) 추세 우수 (펀더멘탈 무관).
...
def pick_fundamental(ind_map: dict[str, dict], info: dict[str, dict],
    ...
    def _scan(relpe_cap: float) -> list[tuple]:
        ...
        for sym, meta in info.items():
            ...
            if not (price > ma20 > ma50 > ma200):    # 정배열
                continue
            mom6 = ind.get("chg_6m")
            if _isnan(mom6) or mom6 <= FUND_MOM_MIN:
                continue
    ...
    picked = pick_with_sector_cap(_scan(FUND_RELPE_MAX), sector_map, n, max(1, RECO_SECTOR_MAX))
    if len(picked) < n and FUND_RELPE_RELAX > FUND_RELPE_MAX:   # 부족 시 밸류 게이트만 1회 완화
        have = {s for s, _, _ in picked}
        extra = [t for t in _scan(FUND_RELPE_RELAX) if t[0] not in have]
        picked += pick_with_sector_cap(extra, sector_map, n - len(picked), max(1, RECO_SECTOR_MAX))
    return picked


def pick_notable(ind_map: dict[str, dict], info: dict[str, dict], vol_stats: dict[str, dict],
                 sector_map: dict[str, str], exclude: set | None = None, n: int = SECTION_N) -> list[tuple]:
    """3) 주목할 종목 (대형 이슈/변동성).
...
def build_final_summary(sec1, sec2, sec3, regime) -> str:
    ...
    mk = []
    if gap is not None and not _isnan(gap):
        mk.append(f"SPY 200일선 {'위' if gap >= 0 else '아래'} {_fmt(gap)}")
    if sc is not None and not _isnan(sc):
        mk.append(f"탐욕지수 {sc:.0f}·{regime.get('fng_rating', '—')}")
    if regime.get("vol_target"):
        mk.append(f"권장 노출 {regime.get('eq_exposure', 1.0) * 100:.0f}%")
...
def _market_overview_block(payload) -> str:
    ...
    rows = [(GICS_KR.get(sec, sec), float(np.median(v))) for sec, v in buckets.items() if len(v) >= 3]
    if not rows:
        return ""
    ...
    def _chips(items, color):
        return " ".join(
            ...
            f'border-radius:10px;padding:1px 8px;margin:1px 3px 1px 0;font-size:12px;'
            f'font-weight:600">{nm} {v:+.1f}%</span>' for nm, v in items)

    up = [(nm, v) for nm, v in rows if v > 0][:4]
    down = [(nm, v) for nm, v in rows[::-1] if v < 0][:4]
    line_up = ('<div style="margin:2px 0"><span style="color:#15803d;font-weight:700;font-size:12px">'
               '▲ 강세</span> ' + _chips(up, "#15803d") + '</div>') if up else ""
    line_dn = ('<div style="margin:2px 0"><span style="color:#b91c1c;font-weight:700;font-size:12px">'
               '▼ 약세</span> ' + _chips(down, "#b91c1c") + '</div>') if down else ""
    if not (line_up or line_dn):
        return ""
    return ('<h3 style="margin:14px 0 2px">🧭 시장 총평 '
            '<span style="color:#9ca3af;font-size:11px">(섹터 추이 · 최근 1주)</span></h3>'
            '<div style="background:#f8fafc;border-left:3px solid #6b7280;padding:8px 12px;'
            f'margin-bottom:6px">{line_up}{line_dn}</div>')
...
def decide_mode(state: dict, data_date: str | None, now_kst=None, force: str | None = None) -> dict:
    ...
    # 데이터가 직전 실행과 동일 ----------------------------------------------------
    if data_current:        # 최신 세션을 이미 보유 = 같은 세션 재실행(휴장 아님)
        if ALLOW_RESEND:    # 재점검용 다회 발송: 평일 4섹션을 그대로 재발송
            return {"mode": "sections", "streak": 0, "data_updated": False,
                    "reason": f"최신 세션({data_date}) 재발송(ALLOW_RESEND) -> 4섹션 추천"}
        return {"mode": "skip_dup", "streak": prev_streak, "data_updated": False,
                "reason": f"최신 세션({data_date}) 이미 처리됨 -> 중복 발송 방지 스킵"}

    # 데이터가 달력상 뒤처짐 = 실제 휴장
    if prev_run_kst == today_kst:       # 같은 KST 날 재실행
        if not ALLOW_RESEND:
            return {"mode": "skip_dup", "streak": prev_streak, "data_updated": False,
                    "reason": "같은 날 재실행(휴장 데이터 동일) -> 중복 발송 방지 스킵"}
        streak = max(prev_streak, 1)    # 재발송 허용: streak 유지(증가 금지)
    else:
...
def build_sections_payload(data: dict):
    ...
    sec3 = pick_notable(ind_map, info, vol_stats, sector_map, exclude=used)       # ③ ①②와 중복 제외
