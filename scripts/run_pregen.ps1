...
#   (2) state/kospi200_cache.json(한국 KRX 데이터 캐시)을 이제 함께 push한다 — 이전엔
...
$log = Join-Path $Root "output\pregen.log"
New-Item -ItemType Directory -Force -Path (Join-Path $Root "output") | Out-Null

# Git attributes select this driver only for state/kospi200_cache.json.
...
function Push-Result {
    ...
    # (2026-09-10 리포 정리) 캐시가 state/로 이동했고, pregen.log는 .gitignore 대상이 돼
    # 커밋 목록에서 뺐다 — 무시되는 파일은 작업트리를 dirty하게 만들지 않으므로 "커밋
    # 안 하면 다음 git pull이 막힌다"(2026-07-23 수정)는 이유 자체가 사라졌다.
    param([string]$CommitMsg)
    $files = @("output/pregen_$Mode.json")
    if ($Mode -eq "kr" -and (Test-Path "state/kospi200_cache.json")) {
        $files += "state/kospi200_cache.json"
    }
