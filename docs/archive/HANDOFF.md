...
> ## ⚠️ 이 문서는 낡았습니다 (2026-07-11 기준) — 믿지 마세요
> 아래 내용은 **폴더 경로(`Documents\stock` → 실제는 `Documents\GitHub\nadsdq100-watcher`),
> 팩터 가중치(`gross_margin×2+accruals×2...` → 폐기됨, 현재 `int_gp_assets:rd_mktcap:
> shareholder_yield=1:2:2`), 리포트 3분류(관찰 티어 → 2026-07-13 폐지)** 가 전부 옛것입니다.
>
> **현재 상태와 전체 인수인계는 [`HISTORY.md`](HISTORY.md)를 보세요** — 최상단 "⭐ 현재
> 라이브 상태 스냅샷"이 지금 실제로 돌아가는 설정의 단일 진실 소스이고, §6이 2026-07-18~22
> 세션(데이터품질 대수술·팩터 재확정·모멘텀 조사·금 재검증)을 담고 있습니다.
>
> 아래 원문은 시스템의 큰 그림(무엇을 하는 시스템인가, 파일 지도, 무료 자원 구성)을 잡는
> 용도로만 참고하세요 — 세부 수치·규칙은 전부 HISTORY.md가 우선입니다.
...
### 전략 개편 (2026-07-07 — 근거는 HISTORY.md 필독)
...
### AI 검증 레이어 (2026-07-08 추가 — HISTORY.md §4.5)
